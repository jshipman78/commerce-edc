...
const iconMap: Record<string, React.ReactNode> = {
  ...
  image: <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21zm16.5-13.5h.008v.008h-.008V7.5zM12 10.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" /></svg>,
};
...
export function AdminShell({ email, children }: { email: string; children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-[#1B2A4A] transition-transform duration-200 lg:static lg:translate-x-0',
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-4">
          <Link href="/admin" className="flex items-center gap-2">
            <span className="text-lg font-bold text-white">Commerce EDC</span>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
